...
**Provider-agnostic by construction — and the server holds no LLM key.** Every chat turn carries the player's own provider + key in `X-LLM-Provider` / `X-LLM-API-Key` headers; `api.py`'s `_user_llm` validates them against `config.PROVIDERS` (Anthropic / OpenAI / OpenRouter, one fixed model each) and `config.build_user_model` constructs the provider object *explicitly* with that key for that one `agent.run(model=...)`. The agent itself is built with `model=None`. Explicit provider objects, not the providers' native env vars, because the key is per-request and anything process-global would leak one player's key into another's turn. There is no `LLM_API_KEY` fallback on purpose: prod would otherwise bill the site owner for strangers. Missing header, unknown provider, and a provider 401/403 (`ModelHTTPError`, caught provider-agnostically via `is_rejected_key`) all surface as one `401` — the frontend's single "fix your key" signal — and in-stream as an `error` event with `code: "bad_key"`. The key is never logged, never in the session cookie (signed, not encrypted), never in the transcript body. The frontend keeps it in `localStorage` only (`llmKey.ts`, same versioned-envelope/try-catch conventions as `savedLoadouts.ts`). `/healthz` `chat` now means "service constructed", not "key present".
